...
`SplashObjcMarkdown`  enables code decoration within Markdown files. The way it works is you simply give it a path to a Markdown file and it will convert any code blocks in it to Splash-highlighted HTML. 
Just like the HTML generated by `SplashObjcHTMLGen` itself, a CSS file should also be added to any page serving the processed Markdown, since Splash only adds CSS classes to tokens — rather than hardcoding styles inline.
